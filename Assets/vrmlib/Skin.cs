...
namespace VrmLib
{
    ...
    public class Skin
    {
        ...
        {
            var dstPosition = vertexBuffer.Positions.GetSpan<Vector3>();
            Span<Vector3> emptyNormal = stackalloc Vector3[0];
            Apply(vertexBuffer, dstPosition, vertexBuffer.Normals != null ? vertexBuffer.Normals.GetSpan<Vector3>() : emptyNormal);
        }
        ...
        public override string ToString()
        {
            if (InverseMatrices != null)
            {
                ...
                var matrices = InverseMatrices.GetSpan<Matrix4x4>();
                var count = 0;
        ...
        public void CalcInverseMatrices()
        {
            ...
            var matricesBytes = new Byte[Marshal.SizeOf(typeof(Matrix4x4)) * Joints.Count].AsMemory();
            var matrices = MemoryMarshal.Cast<byte, Matrix4x4>(matricesBytes.Span);
            ...
            InverseMatrices = new BufferAccessor(matricesBytes, AccessorValueType.FLOAT, AccessorVectorType.MAT4, Joints.Count);
        }

...
namespace VrmLib
{
    public static class ModelExtensionsForCoordinates
    {
                ...
                foreach (var m in g.Meshes)
                {
                    ...
                    switch (m.IndexBuffer.ComponentType)
                    {
                        case AccessorValueType.UNSIGNED_BYTE:
                            FlipTriangle(m.IndexBuffer.GetSpan<byte>());
                            break;
                        case AccessorValueType.UNSIGNED_SHORT:
                            FlipTriangle(m.IndexBuffer.GetSpan<ushort>());
                            break;
                        case AccessorValueType.UNSIGNED_INT:
                            FlipTriangle(m.IndexBuffer.GetSpan<uint>());
                            break;
                        default:
                            throw new NotImplementedException();
                    }
        ...
        static void FlipTriangle(Span<byte> indices)
        {
        ...
        static void FlipTriangle(Span<ushort> indices)
        {
            for (int i = 0; i < indices.Length; i += 3)
            {
                // 0, 1, 2 to 2, 1, 0
        ...
        static void FlipTriangle(Span<uint> indices)
        {
            for (int i = 0; i < indices.Length; i += 3)
            {
                // 0, 1, 2 to 2, 1, 0

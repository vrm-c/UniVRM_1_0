...
namespace VrmLib
{
    public static class ModelExtensionsForBvh
    {
        ...
        {
            // add nodes
            var model = new Model("bvh", Coordinates.Gltf)
            {
        ...
        static Animation LoadAnimation(string name, Bvh.Bvh bvh, Model model, float scalingFactor)
        {
            ...
            var timeSpan = MemoryMarshal.Cast<byte, float>(timeBytes.AsSpan());
            var now = 0.0;
            for (int i = 0; i < timeSpan.Length; ++i, now += bvh.FrameTime.TotalSeconds)
            {
                timeSpan[i] = (float)now;
            }
            var times = new BufferAccessor(timeBytes, AccessorValueType.FLOAT, AccessorVectorType.SCALAR, bvh.FrameCount);
            ...
            foreach (var (key, nodeCurve) in pathMap)
            {
                ...
                if (nodeCurve.LocalPositionX != null)
                {
                    ...
                    var span = MemoryMarshal.Cast<byte, Vector3>(values.AsSpan());
                    for (int i = 0; i < nodeCurve.LocalPositionX.Keys.Length; ++i)
                    {
                        span[i].X = nodeCurve.LocalPositionX.Keys[i] * scalingFactor;
                        span[i].Y = nodeCurve.LocalPositionY.Keys[i] * scalingFactor;
                        span[i].Z = nodeCurve.LocalPositionZ.Keys[i] * scalingFactor;
                    }
                    var sampler = new CurveSampler
                    {
                        In = times,
                        Out = new BufferAccessor(values,
                ...
                if (nodeCurve.EulerX != null)
                {
                    ...
                    var span = MemoryMarshal.Cast<byte, Quaternion>(values.AsSpan());

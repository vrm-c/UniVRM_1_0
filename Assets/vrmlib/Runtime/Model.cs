...
namespace VrmLib
{
    ...
    /// 処理しやすいようにした中間形式
    /// * index 参照は実参照
    /// * accessor, bufferView は実バイト列(Memory<byte>)
        ...
        public override string ToString()
        {
            ...
            if (Vrm != null)
            {
                ...
                if (Vrm.BlendShape != null
                    && Vrm.BlendShape.BlendShapeList != null
                    && Vrm.BlendShape.BlendShapeList.Any())
                {
                    sb.Append("[VRM][blendshape] ");
                    foreach (var ex in Vrm.BlendShape.BlendShapeList)
                    {
                        sb.Append($"[{ex.Preset}]");
                    }
                    sb.Append($"\n");

          ...
          SerializedProperty property, GUIContent label)
        {
            using (new EditorGUI.PropertyScope(position, label, property))
            {
                ...
                if (clip != null)
                {
                    ...
                    var ignoreBlink = clipObj.FindProperty("IgnoreBlink");
                    var ignoreLookAt = clipObj.FindProperty("IgnoreLookAt");
                    var ignoreMouth = clipObj.FindProperty("IgnoreMouth");
                    ...
                    */
                    ...
                    rect.y += (EditorGUIUtility.singleLineHeight + 2);
                    EditorGUI.PropertyField(rect, ignoreBlink);
                    rect.y += (EditorGUIUtility.singleLineHeight + 2);
                    EditorGUI.PropertyField(rect, ignoreLookAt);
                    rect.y += (EditorGUIUtility.singleLineHeight + 2);
                    EditorGUI.PropertyField(rect, ignoreMouth);

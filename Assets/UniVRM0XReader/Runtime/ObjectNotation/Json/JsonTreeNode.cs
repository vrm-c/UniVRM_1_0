...
namespace ObjectNotation
{
    public struct JsonTreeNode
    {
        ...
        {
            if (!(obj is JsonTreeNode))
            {
                return false;
            }

            var rhs = (JsonTreeNode)obj;
        ...
        IEnumerable<string> ToString(string indent, int level, bool value = false)
        {
            ...
            else if (this.IsObject())
            {
                ...
                var isFirst = true;
                var childLevel = level + 1;
                foreach (var kv in this.ObjectItems().OrderBy(x => x.Key.GetString()))
                {
                    if (isFirst)
                    {
                        isFirst = false;
        ...
        public IEnumerable<JsonDiff> Diff(JsonTreeNode rhs, JsonPointer path = default(JsonPointer))
        {
            switch (Value.ValueType)
            {
                case ValueNodeType.Null:
        ...
        public JsonTreeNode Prev
        {
            get
            {
                return new JsonTreeNode(m_Values, ValueIndex - 1);
            }
        }
        ...
        public IEnumerable<JsonTreeNode> Children
        {
            get
            {
                int count = 0;
                for (int i = ValueIndex; count < ChildCount && i < m_Values.Count; ++i)
                {
                    ...
                    {
                        ++count;
                        yield return new JsonTreeNode(m_Values, i);
                    }
                }
            }
        }
        ...
        public JsonTreeNode this[Utf8String key]
        {
            get
            {
        ...
        public JsonTreeNode Parent
        {
            get
            {
                ...
                return new JsonTreeNode(m_Values, Value.ParentIndex);
            }
        }

        public JsonTreeNode(List<JsonValue> values, int index = 0) : this()
        {
            m_Values = values;
            _valueIndex = index;
        }
        ...
        #region JsonPointer
        public JsonTreeNode AddKey(Utf8String key)
        {
        ...
        public JsonTreeNode AddValue(JsonValue value)
        {
            if (m_Values == null)
            {
            ...
            m_Values.Add(value);
            return new JsonTreeNode(m_Values, index);
        }

...
namespace VrmLib
{
    public class VertexBuffer : IEnumerable<KeyValuePair<string, BufferAccessor>>
    {
        ...
        public Span<SkinJoints> GetOrCreateJoints()
        {
            var buffer = Joints;
            if (buffer == null)
            {
                buffer = new BufferAccessor(
                    new byte[Marshal.SizeOf(typeof(SkinJoints)) * Count],
                    AccessorValueType.UNSIGNED_SHORT,
                    AccessorVectorType.VEC4, Count);
                Add(JointKey, buffer);
            }
            return buffer.GetSpan<SkinJoints>();
        }

        public Span<Vector4> GetOrCreateWeights()
        {
            var buffer = Weights;
            if (buffer == null)
            {
                buffer = new BufferAccessor(
                    new byte[Marshal.SizeOf(typeof(Vector4)) * Count],
                    AccessorValueType.FLOAT,
                    AccessorVectorType.VEC4, Count);
                Add(WeightKey, buffer);
            }
            return buffer.GetSpan<Vector4>();
        }
        ...
        public void Append(VertexBuffer v)
        {
            ...
            foreach (var kv in v.VertexBuffers)
            {
                ...
                else
                {
                    ...
                    buffer = new BufferAccessor(new byte[byteLength], kv.Value.ComponentType, kv.Value.AccessorType, lastCount);
                    if (buffer.Count != lastCount)
                    {
                        throw new ArgumentException();
                    }

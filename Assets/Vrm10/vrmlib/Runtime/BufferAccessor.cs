...
namespace VrmLib
{
    ...
    public class BufferAccessor
    {
        ...
        public Span<T> GetSpan<T>(bool checkStride = true) where T : struct
        {
            ...
            return MemoryMarshal.Cast<byte, T>(Bytes.Span);
        }

        public void Assign<T>(Span<T> values) where T : struct
        {
            if (Marshal.SizeOf(typeof(T)) != Stride)
            {
                throw new Exception("invalid element size");
            }
            Bytes = MemoryMarshal.Cast<T, byte>(values).ToArray().AsMemory();
            Count = values.Length;
        }

        // for index buffer
        public void AssignAsShort(Span<int> values)
        {
            ...
            Bytes = new byte[Stride * values.Length];
        ...
        public UShort4[] GetAsUShort4()
        {
            ...
            {
                case AccessorValueType.UNSIGNED_SHORT:
                    return GetSpan<UShort4>().ToArray();

                case AccessorValueType.UNSIGNED_BYTE:
                    {
                        var array = new UShort4[Count];
                        var span = GetSpan<Byte4>();
                        for (int i = 0; i < span.Length; ++i)
                        {
                            array[i].X = span[i].X;
                            array[i].Y = span[i].Y;
        ...
        public Vector4[] GetAsVector4()
        {
            ...
            switch (ComponentType)
            {
                case AccessorValueType.FLOAT:
                    return GetSpan<Vector4>().ToArray();
        ...
        void ToByteLength(int byteLength)
        {
            ...
            var newBytes = new byte[byteLength].AsMemory();
        ...
        public BufferAccessor CloneWithOffset(int offsetCount)
        {
            ...
            var buffer = new byte[offsetSize + Bytes.Length].AsMemory();

            var slice = buffer.Slice(offsetSize);
            Bytes.CopyTo(slice);

            return new BufferAccessor(buffer, ComponentType, AccessorType, Count + offsetCount);
        }

        public BufferAccessor(Memory<byte> bytes, AccessorValueType componentType, AccessorVectorType accessorType, int count)
        {
            Bytes = bytes;
        ...
        public static BufferAccessor Create<T>(T[] list) where T : struct
        {
            ...
            return new BufferAccessor(
                bytes, componentType, accessorType, list.Length);
        }
